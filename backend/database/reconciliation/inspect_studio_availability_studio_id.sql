-- Inspect only studio-availability -> studio FK metadata and aggregate counts.
...
WITH target AS (
    SELECT 'public."StudioAvailabilities"'::regclass AS child_oid,
    ...
    WHERE c.conrelid = t.child_oid
      AND (c.conname = 'FK_StudioAvailabilities_Studios_StudioId'
           OR (c.contype = 'f' AND (c.confrelid = t.parent_oid
           OR starts_with(c.conname::text, 'FK_StudioAvailabilities_Studios_')
...
SELECT jsonb_pretty(jsonb_build_object(
    'expected', jsonb_build_object(
        'child_column', 'public.StudioAvailabilities.StudioId',
        'parent_column', 'public.Studios.Id',
        'column_type', 'uuid', 'both_columns_not_null', true,
        'constraint_name', 'FK_StudioAvailabilities_Studios_StudioId',
    ...
    'preflight_compatible_fk_exists_under_another_name', EXISTS (
        SELECT 1 FROM candidates WHERE exact_relationship
          ...
          AND coalesce((to_jsonb(candidates)->>'conenforced')::boolean, true)
          AND confdeltype = 'c' AND confupdtype = 'a' AND confmatchtype = 's'
          AND conname <> 'FK_StudioAvailabilities_Studios_StudioId'),
    ...
    'relevant_indexes', COALESCE((
        ...
        WHERE attr.attnum = ANY(idx.indkey)
           OR (idx.indrelid = t.child_oid
               AND index_tbl.relname = 'IX_StudioAvailabilities_StudioId_Date')
    ), '[]'::jsonb),
    'index_interpretation', 'The referenced Id requires an eligible unique/primary key. A child StudioId index is not required to create the FK, but the current reconciliation separately requires the unique (StudioId, Date) model index to exist and be compatible.',
    'preflight_comparison', 'format_type(atttypid, atttypmod) <> expected type OR attnotnull <> NOT expected nullable OR attgenerated <> empty string. Expected uuid, nullable=false, not generated. Identity, default and collation are reported but are not compared by this conflict check.',
    ...
    'distinct_studio_ids', COALESCE((
        ...
        FROM (SELECT availability."StudioId"::text AS studio_id, count(*) AS row_count
              FROM public."StudioAvailabilities" availability
              GROUP BY availability."StudioId"::text) grouped
    ), '[]'::jsonb),
    'row_counts', (
        SELECT jsonb_build_object(
            'total_studio_availability_rows', count(*),
        ...
        FROM public."StudioAvailabilities" ps
    )
)) AS studio_availability_studio_id_inspection;
