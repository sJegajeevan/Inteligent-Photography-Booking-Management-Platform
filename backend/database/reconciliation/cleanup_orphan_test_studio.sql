-- ROLLBACK-ONLY rehearsal for explicitly confirmed disposable TEST DATA.
-- Backup reported verified by the operator:
...
BEGIN
    ...
    LOOP
        ...
        END IF;
        RAISE NOTICE 'Dry-run deleted public.%: % rows', r.table_name, deleted_count;
    END LOOP;
    ...
    LOOP
        ...
        verification := verification || jsonb_build_array(jsonb_build_object(
            'table_name', r.table_name, 'status', 'verified before rollback',
...
-- Post-cleanup SELECTs show the transaction's hypothetical final state BEFORE
-- rollback. Original dependent IDs were separately verified above (not rederived
...
SELECT 'DRY RUN ONLY: all preceding deletions will now be rolled back; no reconciliation was applied.' AS result;
ROLLBACK;
