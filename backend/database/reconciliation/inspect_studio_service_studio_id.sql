-- Inspect only studio-service -> studio FK metadata and aggregate counts.
...
WITH target AS (
    SELECT 'public."StudioServices"'::regclass AS child_oid,
    ...
    WHERE c.conrelid = t.child_oid
      AND (c.conname = 'FK_StudioServices_Studios_StudioId'
           OR (c.contype = 'f' AND (c.confrelid = t.parent_oid
           OR starts_with(c.conname::text, 'FK_StudioServices_Studios_')
...
SELECT jsonb_pretty(jsonb_build_object(
    'expected', jsonb_build_object(
        'child_column', 'public.StudioServices.StudioId',
        ...
        'constraint_name', 'FK_StudioServices_Studios_StudioId',
    ...
    'preflight_compatible_fk_exists_under_another_name', EXISTS (
        SELECT 1 FROM candidates WHERE exact_relationship
          ...
          AND coalesce((to_jsonb(candidates)->>'conenforced')::boolean, true)
          AND confdeltype = 'c' AND confupdtype = 'a' AND confmatchtype = 's'
          AND conname <> 'FK_StudioServices_Studios_StudioId'),
    ...
    'relevant_indexes', COALESCE((
        ...
        WHERE attr.attnum = ANY(idx.indkey)
           OR (idx.indrelid = t.child_oid
               AND index_tbl.relname = 'IX_StudioServices_StudioId')
    ), '[]'::jsonb),
    ...
    'distinct_studio_ids', COALESCE((
        ...
        FROM (SELECT service_row."StudioId"::text AS studio_id, count(*) AS row_count
              FROM public."StudioServices" service_row
              GROUP BY service_row."StudioId"::text) grouped
    ), '[]'::jsonb),
    'row_counts', (
        SELECT jsonb_build_object(
            'total_studio_service_rows', count(*),
        ...
        FROM public."StudioServices" ps
    )
)) AS studio_service_studio_id_inspection;
